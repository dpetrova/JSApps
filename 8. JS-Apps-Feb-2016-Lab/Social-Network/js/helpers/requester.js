...
app.requester = (function () {
    ...
    Requester.prototype.get = function (url, useSession) {
        var headers = getHeaders.call(this, false, useSession); //call getHeaders this way
        return makeRequest('GET', url, headers, null);
    };

    Requester.prototype.post = function (url, data, useSession) {
        var headers = getHeaders.call(this, data, useSession); //is data is null == false; if data is object == true
    ...
    function getHeaders(isJSON, useSession) {
        ...
        if (useSession) {
            ...
            headers['Authorization'] = 'Basic ' + btoa(token); //btoa(token) -> encript token
        }
